...
fn solve(m: &Map, t0: usize, source: (usize, usize), target: (usize, usize)) -> usize {
    let mut accessible: HashSet<(usize, usize, usize)> = HashSet::new();
    // accessible[(t, i, j)]: cell (i, j) is accessible at time t

    accessible.insert((t0, source.0, source.1));
...
fn main() {
    ...
    let top_left = (0, 1);
    let bottom_right = (m.lines() - 1, m.cols() - 2);

    let t1 = solve(&m, 0, top_left, bottom_right);
    println!("Arrived to destination at t1={}", style(t1).red());
    let t2 = solve(&m, t1, bottom_right, top_left);
    println!("Arrived back to source at t2={}", style(t2).red());
    let t3 = solve(&m, t2, top_left, bottom_right);
    println!("Arrived back to destination at t3={}", style(t3).red());
}
