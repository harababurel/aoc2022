...
floor_y = max(
    list(map(lambda p: p[1], filter(lambda p: grid[p] == '#',
                                    grid.keys())))) + 2
...
while True:
    ...
    if grid.get((x, y)) == '*':
        print("Sand pile has reached source")
        print("Answer: %d" % successful_units)
    ...
    while True:
        ...
        if y + 1 >= floor_y:
            print("Sand unit has settled on infinite floor")
            successful_units += 1
            break
